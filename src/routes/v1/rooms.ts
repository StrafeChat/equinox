...
router.patch("/:roomId/messages/:messageId", Validator.verifyToken, async (req, res) => {
    try {
        ...
        if (messageQuery.rows[0].get("author_id") != req.body.user.id) return res.status(401).json({ message: "You are not the author of the message you are trying to edit!" });
...
router.delete("/:roomId/messages/:messageId", Validator.verifyToken, async (req, res) => {
    try {
        ...
        if (messageQuery.rows[0].get("author_id") != req.body.user.id) return res.status(401).json({ message: "You are not the author of the message you are trying to delete!" });
